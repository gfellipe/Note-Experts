...
export const NewNoteCard = ({ onNoteCreated }: NewNoteCardProps) => {
  ...
  const handleStartRecording = () => {
    ...
    speechRecognition.onresult = (event) => {
      const trancripiton = Array.from(event.results).reduce((text, result) => {
        return text.concat(result[0].transcript);
      }, "");

      setContent(trancripiton);
    };
  ...
  return (
    <Dialog.Root>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed overflow-hidden inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[525px] w-full md:h-[70vh] bg-slate-700 md:rounded-md flex flex-col outline-none">
